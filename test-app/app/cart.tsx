...
import { cartItems, clearCart, removeFromCart, updateCartItemQuantity, createOrder } from './cartStore';
...
// Mock data for delivery orders in Indian Rupees
const DELIVERY_ORDERS: DeliveryOrder[] = [
  {
    id: '1',
    orderNumber: 'ORD-001',
    customerName: 'Rahul Sharma',
    address: 'A-101, Green Valley Apartments, Sector 15',
    items: ['Organic Bananas (2kg)', 'Fresh Milk (1L)', 'Whole Wheat Bread'],
    total: 980,
    status: 'on_the_way',
  ...
  },
  {
    ...
    address: 'B-205, Skyline Towers, MG Road',
    items: ['Chicken Breast (500g)', 'Basmati Rice (2kg)', 'Tomatoes (1kg)'],
    total: 1310,
    status: 'preparing',
  ...
  },
  {
    ...
    address: 'C-304, Lakeview Apartments, Koramangala',
    items: ['Pasta (500g)', 'Pasta Sauce', 'Mozzarella Cheese'],
    total: 756,
    status: 'pending',
  ...
  },
  {
    ...
    address: 'D-12, Tech Park Residence, Whitefield',
    items: ['Salmon Fillet', 'Asparagus', 'Lemon', 'Olive Oil'],
    total: 1808,
    status: 'delivered',
...
const CartScreen: React.FC = () => {
  ...
  const getStatusInfo = (status: string) => {
    switch (status) {
      case 'pending':
        return { color: '#FFA500', icon: '⏳', text: 'Pending' };
      case 'preparing':
        return { color: '#007ACC', icon: '👨‍🍳', text: 'Preparing' };
      case 'on_the_way':
        return { color: '#4CAF50', icon: '🚚', text: 'On the way' };
      case 'delivered':
        return { color: '#666', icon: '✅', text: 'Delivered' };
      default:
        return { color: '#999', icon: '📦', text: 'Unknown' };
    }
  };

  // Render delivery order item
  const renderDeliveryOrderItem = ({ item }: { item: DeliveryOrder }) => {
    const statusInfo = getStatusInfo(item.status);
    
    return (
      <View style={styles.deliveryOrderItem}>
        ...
        <View style={styles.orderDetails}>
          <Text style={styles.orderAddress} numberOfLines={2}>{item.address}</Text>
  ...
  return (
    <SafeAreaView style={styles.container}>
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <TouchableOpacity 
              ...
              onPress={() => setShowAllOrdersModal(true)}
            >
          ...
          <View style={styles.activeOrdersCard}>
            <Text style={styles.activeOrdersTitle}>Active Deliveries</Text>
            <View style={styles.ordersStats}>
              <View style={styles.statItem}>
                <Text style={styles.statNumber}>{DELIVERY_ORDERS.filter(order => order.status === 'on_the_way').length}</Text>
                <Text style={styles.statLabel}>On the way</Text>
              </View>
              <View style={styles.statItem}>
                <Text style={styles.statNumber}>{DELIVERY_ORDERS.filter(order => order.status === 'preparing').length}</Text>
                <Text style={styles.statLabel}>Preparing</Text>
              </View>
              <View style={styles.statItem}>
                <Text style={styles.statNumber}>{DELIVERY_ORDERS.filter(order => order.status === 'pending').length}</Text>
      ...
      {/* All Orders Modal */}
      <Modal visible={showAllOrdersModal} animationType="slide" transparent>
        <View style={styles.modalContainer}>
          <View style={styles.allOrdersModalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>All Delivery Orders</Text>
              <TouchableOpacity 
                style={styles.closeModalIcon}
                onPress={() => setShowAllOrdersModal(false)}
              >
                <Text style={styles.closeModalIconText}>×</Text>
              </TouchableOpacity>
            </View>
            
            <View style={styles.ordersSummary}>
              <Text style={styles.ordersSummaryText}>
                Total Orders: {DELIVERY_ORDERS.length}
              </Text>
            </View>

            <FlatList
              data={DELIVERY_ORDERS}
              renderItem={renderDeliveryOrderItem}
              keyExtractor={item => item.id}
