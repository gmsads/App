...
const ProductItem = ({ 
  ...
  const handleAddButtonPress = () => {
    if (item.availableUnits && item.availableUnits.length > 1) {
      ...
      setModalQuantity(1);
      if (!selectedUnitId && item.availableUnits.length > 0) {
        setSelectedUnitId(item.availableUnits[0].id);
      }
    } else {
      addToCart({
        id: item.id,
  ...
  const handleConfirmAddToCart = () => {
    const selectedUnit = item.availableUnits?.find(u => u.id === selectedUnitId) || currentUnit;
    ...
    Alert.alert(
      'Added to Cart',
      `${modalQuantity} ${selectedUnit.unit} of ${item.name} added to cart!`,
      [
        { 
          text: 'OK', 
          style: 'default',
    ...
    <>
      ...
      >
        <View style={styles.productImageContainer}>
          ...
          >
            <Text style={styles.addButtonOnImageText}>ADD</Text>
          </TouchableOpacity>
        ...
        <View style={styles.newModalOverlay}>
          <View style={styles.newModalContent}>
            <View style={styles.newModalHeader}>
              <Text style={styles.newModalTitle}>Select Unit</Text>
              <TouchableOpacity 
                style={styles.newModalCloseBtn}
                onPress={() => setShowUnitModal(false)}
              >
                <Text style={styles.newModalClose}>✕</Text>
              </TouchableOpacity>
            </View>

            <View style={styles.newModalProductInfo}>
              <Image source={{ uri: item.image }} style={styles.newModalProductImage} />
              <View style={styles.newModalProductDetails}>
                <Text style={styles.newModalProductName}>{item.name}</Text>
                <View style={styles.newModalPriceRow}>
                  <Text style={styles.newModalCurrentPrice}>₹{currentUnit.price}</Text>
                  {currentUnit.originalPrice && (
                    <Text style={styles.newModalOriginalPrice}>₹{currentUnit.originalPrice}</Text>
                  )}
                  {currentUnit.discount && currentUnit.discount > 0 && (
                    <View style={styles.newModalDiscountBadge}>
                      <Text style={styles.newModalDiscountText}>
            ...
            <View style={styles.newUnitSelectionContainer}>
              <Text style={styles.newUnitSectionTitle}>Available Units</Text>
              
              {item.availableUnits?.map((unit) => (
                <TouchableOpacity
                  key={unit.id}
                  style={[
                    styles.newUnitOption,
                    selectedUnitId === unit.id && styles.newUnitOptionSelected
                  ]}
                  onPress={() => handleUnitSelect(unit)}
                >
                  <View style={styles.newUnitLeft}>
                    <Text style={[
                      styles.newUnitLabel,
                      selectedUnitId === unit.id && styles.newUnitLabelSelected
                    ]}>
                      {unit.unit}
                    </Text>
                    {unit.discount && unit.discount > 0 && (
                      <View style={styles.newUnitSaveBadge}>
                        <Text style={styles.newUnitSaveText}>
                  ...
                  <View style={styles.newUnitRight}>
                    <View style={styles.newUnitPriceContainer}>
                      <Text style={[
                        styles.newUnitPrice,
                        selectedUnitId === unit.id && styles.newUnitPriceSelected
                      ]}>
                        ₹{unit.price}
                      </Text>
                      {unit.originalPrice && (
                        <Text style={styles.newUnitOriginalPrice}>
                          ₹{unit.originalPrice}
                        </Text>
                      )}
                    </View>
                    ...
                    {selectedUnitId === unit.id && (
                      <View style={styles.newUnitSelectedIndicator}>
                        <Text style={styles.newUnitSelectedIndicatorText}>✓</Text>
                      </View>
                    )}
                  </View>
                </TouchableOpacity>
            ...
            <View style={styles.newQuantityTotalContainer}>
              <View style={styles.newQuantityContainer}>
                <Text style={styles.newQuantityLabel}>Quantity</Text>
                <View style={styles.newQuantitySelector}>
                  <TouchableOpacity 
                    style={styles.newQuantityButton} 
                    onPress={decreaseModalQuantity}
                  >
                    <Text style={styles.newQuantityButtonText}>-</Text>
                  </TouchableOpacity>
                  <Text style={styles.newQuantityValue}>{modalQuantity}</Text>
                  <TouchableOpacity 
                    style={styles.newQuantityButton} 
              ...
              <View style={styles.newTotalContainer}>
                ...
                <Text style={styles.newTotalPrice}>
                  ₹{(selectedUnitId ? 
                    (item.availableUnits?.find(u => u.id === selectedUnitId)?.price || currentUnit.price) * modalQuantity 
                    : currentUnit.price * modalQuantity).toFixed(2)}
                </Text>
              </View>
            </View>

            <TouchableOpacity 
              style={styles.newAddToCartButton}
              onPress={handleConfirmAddToCart}
            >
              <Text style={styles.newAddToCartButtonText}>ADD TO CART</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
...
const ProductList: React.FC = () => {
  ...
  const [shopInfo] = useState<ShopInfo>({
    name: 'Road No 1, RTC Colony, LB Nagar, Suvid...',
    vendor: 'Venkat Packy',
    address: 'Location - 1'
  });

  const [deliveryTime] = useState('46 minutes');
  ...
  const categoryTabItems = [
    { id: '1', name: 'All', value: 'all', icon: '🛒' },
    { id: '2', name: 'Veggies', value: 'vegetables', icon: '🥬' },
    { id: '3', name: 'Fruits', value: 'fruits', icon: '🍎' },
    { id: '4', name: 'Exotics', value: 'exotic', icon: '🥑' },
    { id: '5', name: 'Cuts', value: 'cuts', icon: '🥗' },
    { id: '6', name: 'Herbs', value: 'herbs', icon: '🌿' },
    { id: '7', name: 'Flowers', value: 'flowers', icon: '💐' },
    { id: '8', name: 'Organic', value: 'organic', icon: '🌱' },
  ];

  // Subcategories for the left sidebar
  const subCategoryItems = [
    { id: 'sc1', name: 'All', value: 'all', icon: '📦' },
    { id: 'sc2', name: 'Fresh', value: 'fresh', icon: '🌱' },
    { id: 'sc3', name: 'Vegetable...', value: 'vegetables', icon: '🥬' },
    { id: 'sc4', name: 'Fruits', value: 'fruits', icon: '🍎' },
    { id: 'sc5', name: 'New Launches', value: 'new', icon: '🆕' },
    { id: 'sc6', name: 'Herbs', value: 'herbs', icon: '🌿' },
    { id: 'sc7', name: 'Exotics', value: 'exotic', icon: '🥑' },
    { id: 'sc8', name: 'Organic', value: 'organic', icon: '🌱' },
  ];
  ...
  const [products] = useState<Product[]>([
    {
      ...
      unit: '100 g',
      availableUnits: [
        { id: 'unit_7_1', unit: '100 g', price: 10, originalPrice: 13, discount: 23 },
        { id: 'unit_7_2', unit: '2 x 100 g', price: 19, originalPrice: 26, discount: 26 },
      ]
    },
    {
      id: '2',
    ...
    if (query === '') {
      if (activeTab === 'home' || activeTab === 'categories' || activeTab === 'reorder') {
        ...
        const matchesSubCategory = selectedSubCategory === 'all' || 
          (selectedSubCategory === 'fresh' && ['vegetables', 'fruits', 'herbs'].includes(product.category)) ||
          (selectedSubCategory === 'new' && ['exotic', 'organic'].includes(product.category)) ||
    ...
    return (
      ...
      >
        <View style={styles.topSellerImageContainer}>
          ...
          <TouchableOpacity 
            style={styles.topSellerAddButton}
            onPress={handleAddButtonPress}
          >
            <Text style={styles.topSellerAddButtonText}>ADD</Text>
          </TouchableOpacity>
          
          {item.originalPrice && (
            <View style={styles.topSellerOfferBadge}>
              <View style={styles.topSellerOfferLine}></View>
              <Text style={styles.topSellerOfferText}>
                ₹{Math.round(item.originalPrice - item.price)} OFF
              </Text>
            </View>
          )}
          {item.discount && (
        ...
        <View style={styles.topSellerInfo}>
          ...
          <View style={styles.topSellerUnitRow}>
            <Text style={styles.topSellerUnit}>{item.unit}</Text>
          </View>
          
          <View style={styles.topSellerDeliveryContainer}>
            <Text style={styles.topSellerDeliveryTime}>49 mins</Text>
          </View>
        </View>
      </TouchableOpacity>
    );
    ...
    <View style={styles.newUIStructure}>
      <View style={styles.topBar}>
        ...
        <TouchableOpacity 
          ...
          onPress={() => router.push('/cart')}
        >
          <Text style={styles.cartIconTop}>🛒</Text>
          {cartItemCount > 0 && (
            <View style={styles.cartBadgeTop}>
              <Text style={styles.cartBadgeTextTop}>{cartItemCount}</Text>
            </View>
          )}
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.shopInfoContainer}>
        ...
        <View style={styles.vendorInfo}>
          ...
          <Text style={styles.vendorName}>{shopInfo.vendor}</Text>
        </View>
      ...
      <View style={styles.searchBarNew}>
        ...
        <TextInput
          ...
          placeholder="Search for 'Nano Banana'"
      ...
      <View style={styles.welcomeOffersContainer}>
        <View style={styles.welcomeOfferContainer}>
          <View style={styles.welcomeOfferItem}>
            <Text style={styles.welcomeOfferTitle}>Choose Delivery Day</Text>
            <View style={styles.welcomeDeliveryOptions}>
              <TouchableOpacity 
                style={[
                  styles.welcomeDeliveryOption,
                  deliveryType === 'NEXT_DAY' && styles.welcomeDeliveryOptionActive
                ]}
                onPress={() => setDeliveryType('NEXT_DAY')}
              >
                <Text style={[
                  styles.welcomeDeliveryText,
                  deliveryType === 'NEXT_DAY' && styles.welcomeDeliveryTextActive
                ]}>
                  3hr Delivery
                </Text>
              </TouchableOpacity>
              <TouchableOpacity 
                style={[
                  styles.welcomeDeliveryOption,
                  deliveryType === 'SAME_DAY' && styles.welcomeDeliveryOptionActive
                ]}
                onPress={() => setDeliveryType('SAME_DAY')}
              >
                <Text style={[
                  styles.welcomeDeliveryText,
                  deliveryType === 'SAME_DAY' && styles.welcomeDeliveryTextActive
                ]}>
                  Next Day Delivery
                </Text>
              </TouchableOpacity>
  ...
  // Render top categories (horizontal)
  const renderTopCategories = () => (
    <View style={styles.topCategoriesContainer}>
      <Text style={styles.topCategoriesTitle}>Fruits & Vegetables</Text>
      <ScrollView 
        horizontal 
        showsHorizontalScrollIndicator={false}
        style={styles.topCategoriesScroll}
        ...
        {mainCategories.map((category) => (
          <TouchableOpacity
            ...
            style={[
              ...
              selectedCategory === category.value && styles.topCategoryItemActive
            ]}
            onPress={() => handleCategorySelect(category.value as Category)}
          >
            <Text style={[
              styles.topCategoryText,
              selectedCategory === category.value && styles.topCategoryTextActive
            ]}>
              {category.name}
            </Text>
          </TouchableOpacity>
  ...
  // Render left sidebar with round buttons
      ...
      >
        {subCategoryItems.map((subCategory) => (
          <TouchableOpacity
            ...
            onPress={() => handleSubCategorySelect(subCategory.value)}
          >
            <View style={[
              styles.subCategoryIconCircle,
              selectedSubCategory === subCategory.value && styles.subCategoryIconCircleActive
            ]}>
              <Text style={styles.subCategoryIcon}>{subCategory.icon}</Text>
            </View>
            <Text style={[
              styles.subCategoryName,
              selectedSubCategory === subCategory.value && styles.subCategoryNameActive
            ]}>
              {subCategory.name}
            </Text>
          </TouchableOpacity>
    ...
    return (
      <View style={styles.categoriesTabLayout}>
        ...
        <View style={styles.categoriesMainContent}>
          {renderSubCategories()}
          <View style={styles.categoriesProductsContainer}>
            <View style={styles.categoriesProductsHeader}>
              <Text style={styles.categoriesProductsTitle}>
            ...
            {filteredProducts.length > 0 ? (
              <FlatList
                ...
                numColumns={2}
                showsVerticalScrollIndicator={false}
                contentContainerStyle={styles.categoriesProductsGrid}
              />
            ) : (
  ...
  const handleTopSellerPress = (item: TopSeller) => {
    const product: Product = {
      id: item.id,
      name: item.name,
      price: item.price,
      image: item.image,
      description: `${item.name} - Top Seller`,
      category: 'fruits',
      unit: item.unit,
      discount: item.discount,
      originalPrice: item.originalPrice
              ...
              <View style={styles.topSellersSection}>
                ...
                <FlatList
                  ...
                  renderItem={({ item }) => (
                    <TopSellerItem 
                      item={item} 
                      onPress={() => handleTopSellerPress(item)}
                      onAddToCart={updateCartCount}
                    />
                  )}
                  keyExtractor={(item) => item.id}
...
const styles = StyleSheet.create({
  ...
  categoriesProductsHeader: {
    paddingHorizontal: 16,
    paddingVertical: 12,
    borderBottomWidth: 1,
    borderBottomColor: '#e0e0e0',
  },
  categoriesProductsTitle: {
    fontSize: 16,
    fontWeight: 'bold',
    color: '#2c3e50',
  },
  ...
  topSellerDeliveryContainer: {
    flexDirection: 'row',
    alignItems: 'center',
  },
  topSellerDeliveryTime: {
    fontSize: 11,
    color: '#666',
    fontWeight: '500',
  },
  ...
  newModalCurrentPrice: {
    fontSize: 20,
    fontWeight: 'bold',
    color: '#e74c3c',
    marginRight: 8,
  },
  newModalOriginalPrice: {
    fontSize: 16,
    color: '#999',
    textDecorationLine: 'line-through',
    marginRight: 8,
  },
  newModalDiscountBadge: {
    backgroundColor: '#e74c3c',
    paddingHorizontal: 8,
    paddingVertical: 4,
    borderRadius: 4,
  },
  newModalDiscountText: {
    fontSize: 12,
    color: 'white',
    fontWeight: 'bold',
  },
  newUnitSelectionContainer: {
    paddingVertical: 16,
  },
  newUnitSectionTitle: {
    fontSize: 14,
  ...
  newUnitPriceSelected: {
    color: '#27ae60',
  },
  newUnitOriginalPrice: {
    fontSize: 12,
    color: '#999',
    textDecorationLine: 'line-through',
    marginTop: 2,
  },
  newUnitSelectedIndicator: {
    width: 20,
    height: 20,
    borderRadius: 10,
    backgroundColor: '#27ae60',
  ...
  newUnitSelectedIndicatorText: {
    fontSize: 12,
    color: 'white',
    fontWeight: 'bold',
  },
  newQuantityTotalContainer: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
    paddingVertical: 16,
