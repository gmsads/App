...
type Product = {
  ...
  subCategory: string; // readonly
  sameDayAvailable: string; // editable
  nextDayAvailable: string; // editable
  quantityOptions: {
    [key: string]: QuantityOptionDTO[]; // editable
  };
};
...
const CARD_MARGIN = 4;
const OPTION_CARD_WIDTH = (screenWidth - 24 - (CARD_MARGIN * 4)) / 3;
...
const ProductList = () => {
  const [products, setProducts] = useState<Product[]>([
    {
      ...
      quantityOptions: {
        sameDay: [
          ...
          { 
            quantity: '2', unit: 'kg', actualPrice: 150, salePrice: 120, discountPercent: 20, 
            isEnabled: true, isOutOfStock: false 
          },
        ],
        nextDay: [
    ...
    },
    {
      ...
      quantityOptions: {
        sameDay: [
          ...
          { 
            quantity: '500', unit: 'g', actualPrice: 70, salePrice: 60, discountPercent: 14, 
            isEnabled: true, isOutOfStock: false 
          },
        ],
        nextDay: [
    ...
    },
    {
      ...
      quantityOptions: {
        sameDay: [
          ...
          { 
            quantity: '500', unit: 'ml', actualPrice: 35, salePrice: 32, discountPercent: 9, 
            isEnabled: true, isOutOfStock: false 
          },
          { 
            quantity: '2', unit: 'litre', actualPrice: 110, salePrice: 99, discountPercent: 10, 
            isEnabled: false, isOutOfStock: false 
          },
        ],
        nextDay: [
  ...
  const [editedOption, setEditedOption] = useState<QuantityOptionDTO | null>(null);
  ...
  // Function to fetch product data from API when edit is clicked
  const fetchProductData = async (shopProductId: string) => {
    setFetchingProduct(true);
    try {
      // Simulating API call - Replace with actual API call
      // const response = await api.get(`/products/${shopProductId}`);
      // const productData = response.data;
      
      // Mock API response - In real app, use the actual API response
      await new Promise(resolve => setTimeout(resolve, 500)); // Simulate network delay
      
      // Find product in local state (in real app, this would come from API)
      const productFromApi = products.find(p => p.shopProductId === shopProductId);
      
      if (productFromApi) {
        setSelectedProduct(productFromApi);
        // Set initial editing option if available
        if (productFromApi.quantityOptions?.sameDay?.[0]) {
          setEditingOptionIndex(0);
          setEditedOption({...productFromApi.quantityOptions.sameDay[0]});
        }
  ...
  const handleEditProduct = (product: Product) => {
    ...
    fetchProductData(product.shopProductId);
  ...
  const handleEditOption = (product: Product, type: 'sameDay' | 'nextDay', index: number) => {
    ...
    const optionToEdit = product.quantityOptions[type]?.[index];
    if (optionToEdit) {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const updatePayload = {
        ...
        sameDayAvailable: selectedProduct.sameDayAvailable,
        nextDayAvailable: selectedProduct.nextDayAvailable,
        quantityOptions: {
          ...selectedProduct.quantityOptions,
          [editOptionsType]: [
            ...selectedProduct.quantityOptions[editOptionsType].map((opt, idx) => 
              idx === editingOptionIndex ? editedOption : opt
            )
          ]
        }
      };
      ...
      // Simulating API call - Replace with actual API call
      // await api.put(`/products/${selectedProduct.shopProductId}`, updatePayload);
      await new Promise(resolve => setTimeout(resolve, 500)); // Simulate network delay
      
      // Update local state (in real app, this would be handled by refetching or updating cache)
      const updatedProducts = products.map(product => {
        if (product.shopProductId === selectedProduct.shopProductId) {
          return {
            ...product,
            sameDayAvailable: updatePayload.sameDayAvailable,
            nextDayAvailable: updatePayload.nextDayAvailable,
            quantityOptions: updatePayload.quantityOptions
          };
        }
        return product;
      });
  ...
  const renderProductCard = ({ item }: { item: Product }) => {
    return (
      <View style={styles.productCard}>
        ...
        {/* Same Day Options - 3 cards in a row */}
        <View style={styles.sectionContainer}>
          ...
          <View style={styles.optionsRow}>
            {item.quantityOptions?.sameDay?.map((option, index) => (
                    ...
                    <Switch
                      ...
                      onValueChange={() => {
                        const updatedProducts = products.map(p => {
                          if (p.shopProductId === item.shopProductId) {
                            const updated = {...p};
                            updated.quantityOptions.sameDay[index].isEnabled = !option.isEnabled;
                            return updated;
                          }
                          return p;
                        });
        ...
        {/* Next Day Options - 3 cards in a row */}
        <View style={styles.sectionContainer}>
          <View style={styles.sectionHeader}>
          ...
          <View style={styles.optionsRow}>
            {item.quantityOptions?.nextDay?.map((option, index) => (
              <View key={index} style={styles.optionItem}>
                <View style={styles.optionHeader}>
                  <View style={styles.switchContainer}>
                    <Switch
                      ...
                      onValueChange={() => {
                        const updatedProducts = products.map(p => {
                          if (p.shopProductId === item.shopProductId) {
                            const updated = {...p};
                            updated.quantityOptions.nextDay[index].isEnabled = !option.isEnabled;
                            return updated;
                          }
                          return p;
                        });
            ...
            {/* Add empty views to fill the row if less than 3 options */}
            {item.quantityOptions?.nextDay && item.quantityOptions.nextDay.length < 3 && 
              Array.from({ length: 3 - item.quantityOptions.nextDay.length }).map((_, index) => (
                <View key={`empty-${index}`} style={styles.emptyOptionItem} />
              ))
            }
          </View>
        </View>
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        keyExtractor={(item) => item.shopProductId}
              ...
              {fetchingProduct ? (
                ...
                <>
                  ...
                  <View style={styles.readOnlySection}>
                    <Text style={styles.readOnlyTitle}>Product Information (Read Only)</Text>
                    <View style={styles.readOnlyRow}>
                      <Text style={styles.readOnlyLabel}>Product ID:</Text>
                      <Text style={styles.readOnlyValue}>{selectedProduct.productId}</Text>
                    </View>
                    <View style={styles.readOnlyRow}>
                      <Text style={styles.readOnlyLabel}>Name:</Text>
                      <Text style={styles.readOnlyValue}>{selectedProduct.name}</Text>
                    </View>
                    <View style={styles.readOnlyRow}>
                      <Text style={styles.readOnlyLabel}>Category:</Text>
                      <Text style={styles.readOnlyValue}>{selectedProduct.category}</Text>
                    </View>
                    <View style={styles.readOnlyRow}>
                      <Text style={styles.readOnlyLabel}>Sub Category:</Text>
                      <Text style={styles.readOnlyValue}>{selectedProduct.subCategory}</Text>
                    </View>
                  </View>

                  {/* Editable Fields */}
                  <View style={styles.editableSection}>
                    <Text style={styles.editableTitle}>Availability Settings (Editable)</Text>
                    
                    <View style={styles.availabilityRow}>
                      <Text style={styles.availabilityLabel}>Same Day Available:</Text>
                      <Switch
                        value={selectedProduct.sameDayAvailable === 'true'}
                        onValueChange={(value) => {
                          setSelectedProduct({
                            ...selectedProduct,
                            sameDayAvailable: value ? 'true' : 'false'
                          });
                        }}
                        trackColor={{ false: '#ddd', true: '#4CAF50' }}
                        ios_backgroundColor="#ddd"
                    ...
                    <View style={styles.availabilityRow}>
                      ...
                      <Switch
                        value={selectedProduct.nextDayAvailable === 'true'}
                        onValueChange={(value) => {
                          setSelectedProduct({
                            ...selectedProduct,
                            nextDayAvailable: value ? 'true' : 'false'
                          });
                        }}
                ...
                <View style={styles.errorContainer}>
                  <Text style={styles.errorText}>Failed to load product data</Text>
                </View>
              )}
            </ScrollView>
